...
**Tab bar**: a fixed bottom `.tabbar` with three tabs — Home, Looks, Account — toggles `#home-view` / `#looks-view` / `#account-view` via `switchTab()`, following the same `.hidden`-class pattern as everything else (no router). The tab bar itself is hidden during the focused loading/live-guide flow (`setTabbarVisible(false)`) and restored on the done screen or when returning to setup, so it can't be used to abandon an in-progress guide.

**App state machine** (inside `#home-view`): a single global `state` object (`occasion`, `products`, `steps`, `currentStep`, `stream`) drives four screens toggled via the `.hidden` class — `#setup-screen` → `#loading-screen` → `#live-screen` → `#done-screen`. DOM elements are looked up by ID on demand rather than cached.
